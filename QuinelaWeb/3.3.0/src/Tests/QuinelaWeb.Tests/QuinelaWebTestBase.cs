...
namespace QuinelaWeb.Tests
{
    public abstract class QuinelaWebTestBase : AbpIntegratedTestBase<QuinelaWebTestModule>
    {
        ...
        protected void UsingDbContext(Action<QuinelaWebDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<QuinelaWebDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<QuinelaWebDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<QuinelaWebDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<QuinelaWebDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<QuinelaWebDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<QuinelaWebDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<QuinelaWebDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<QuinelaWebDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<QuinelaWebDbContext, Task<T>> func)
        {
            T result;
